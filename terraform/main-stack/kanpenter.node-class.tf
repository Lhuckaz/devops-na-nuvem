...
resource "kubernetes_manifest" "node_class" {
  manifest = yamldecode(templatefile("./manifest/kanpenter.node-class.yaml", { node_group_role_name = aws_iam_role.eks_node_group.name, cluster_name = aws_eks_cluster.this.id }))
  depends_on = [
    aws_eks_node_group.this
  ]
}
